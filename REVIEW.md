...
- 实际代码：
  - `setup()` 内部直接初始化 Wi-Fi/ESP-NOW，并注册回调，LED 也在回调首次收到主机消息后切换为绿色，缺少单元测试模式的隔离。【F:slave_main.cpp†L82-L137】【F:slave_main.cpp†L337-L407】
  - 全局不存在 `current_command` 与 `new_command`，所有命令均来自 `onDataRecv()`；`loop()` 也已经运行完整的运行状态机而非预期的模拟触发流程。【F:slave_main.cpp†L307-L333】【F:slave_main.cpp†L365-L420】
...
- 实际代码：
  - 从机始终处于网络模式，没有保留与第一阶段之间的切换逻辑，导致无法单独运行单元测试版本。【F:slave_main.cpp†L307-L355】
  - 主机端实现了完整的展演流程：
...
## 当前固件的完整运行流程
1. **从机启动**：
   - `setup()` 初始化 Wi-Fi、ESP-NOW、步进电机及 LED。LED 默认红色，首次接收到主机消息后切换为绿色并进入常规位置指示模式。【F:slave_main.cpp†L337-L369】【F:slave_main.cpp†L300-L333】
   - 主循环 `loop()` 根据 `system_state` 执行归位、运行、随机等状态，同时定时发送遥测（状态码 5）并在触发点上报状态码 4。【F:slave_main.cpp†L365-L420】
